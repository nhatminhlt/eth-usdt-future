...
| Chi phí RT ≈ 0.7–1.2 pip (≈0.006–0.011%) ≈ **5–8% R** | Exness spread+slip | **Taker RT ≈ 0.09–0.11% ≈ 12–24% R; Maker RT ≈ 0.035–0.055% ≈ 5–11% R** (đã tính BNB −10%) | ⚠️ Khác biệt bậc chi phí — xem mục 11, luật 1 |
...
- **Execution là biến số tối ưu hóa, không định kiến (2026-09-20 — user gỡ ràng buộc maker-first):** maker hay taker đều được, tiêu chí duy nhất là **net expectancy sau chi phí thực**. Quy trình: mỗi chiến thuật chạy cả biến thể entry maker (post-only/entry-retest) lẫn taker (stop-market) qua funnel; chọn biến thể thắng theo net expectancy của kịch bản cost tương ứng. Bài học V1 giữ nguyên dạng con số tham chiếu: taker RT ≈ 0.09–0.11% ≈ 12–24% R — chiến thuật taker phải tự gánh được bậc chi phí này; maker không được dùng để "cứu" chiến thuật gross âm (luật 2). Funnel báo cáo maker-share, fill-rate post-only, non-fill rate song song với PnL. *Lưu ý cơ chế: post-only đặt ngay tại cực trị chỉ khớp khi giá quay lại chạm (adverse selection có chủ đích) — biến thể maker khả dĩ cho breakout là entry-retest (định nghĩa ở S1).*
...
- `costs.py` — **mọi phí theo giả định BNB: maker 0.018% / taker 0.045%**; 2 kịch bản song song, **không định trước kịch bản chính** — kịch bản "chính" của mỗi chiến thuật = execution mà chiến thuật đó sẽ trade thật; kịch bản còn lại vẫn báo cáo đầy đủ:
  - `maker-base`: entry post-only limit tại trigger (mô hình **non-fill + adverse selection** — bài học V1 mục 9.5.5; fill chỉ tính khi trade-through — fill rule ở engine M4), TP limit maker 0.018%, SL taker 0.045% + slippage. RT kỳ vọng ≈ 0.035–0.055%.
...
- Expectancy sau chi phí **trong kịch bản execution được chọn cho chiến thuật** > 0.05R (bootstrap CI không chứa 0); kịch bản execution còn lại báo cáo minh bạch — chấp nhận âm nhẹ nhưng phải ghi rõ điều kiện sống.
...
- **Chi phí giết edge** (bằng chứng V1) → execution là quyết định của dữ liệu: mỗi chiến thuật so sánh biến thể maker vs taker trong funnel, chọn theo net expectancy; gate gross-trước-phí + alpha decomposition vẫn bắt buộc (gross âm thì không kịch bản execution nào cứu được).
...
1. **Net-after-cost là tiêu chí tối thượng; execution (maker/taker) là biến số tối ưu hóa, không định kiến** (2026-09-20). Bài học V1 giữ nguyên dạng số học: taker RT ≈ 12–24% R → chiến thuật taker phải chứng minh gross đủ lớn để trả; gross âm thì không execution nào cứu được (luật 2). Fill-model của kịch bản nào thì kết quả kịch bản đó tính theo — không được mix maker-fill lạc quan với cost taker.
...
- **Vòng research 2 (2026-09-20):** Risk-managed momentum tăng Sharpe — Yang 2025: [sciencedirect.com](https://www.sciencedirect.com); Dynamic TSMOM crypto — Borgards 2021 (cited 32); tail/vol-managed momentum — Grobys 2025; Bitcoin Options Risk-Reversal Predictability — Neo 2026: [ink.library.smu.edu.sg](https://ink.library.smu.edu.sg); 25Δ RR thực chiến — Deribit Insights: [insights.deribit.com](https://insights.deribit.com); Fundamentals of Perpetual Futures — arXiv 2024 (funding biến động mạnh hơn sau mốc): [arxiv.org](https://arxiv.org); cross-exchange price discovery — Albers 2021 (Binance leads): [tandfonline.com](https://www.tandfonline.com); Zhivkov 2026: [mdpi.com](https://www.mdpi.com). **Dữ liệu verify trực tiếp trên data.binance.vision cho SOLUSDT (S3, 2026-09-20 — SỬA LẠI: verify bằng probe HTTP thật, `liquidationSnapshot` 404 KHÔNG TỒN TẠI — claim verify trước đó là sai; `metrics/` và `bookDepth/` (daily) và `aggTrades/` là 200 OK. H1 cascade fade dùng proxy OI-drop + volume/giá spike như đã dự phòng ở caveat.)**
...
- **Vòng research 4 (2026-09-20 — đóng khoảng trống cuối + gỡ ràng buộc maker):** Biểu phí VIP0 2026 xác nhận không đổi (0.02/0.05, BNB −10%): [binance.com/en/fee/futureFee](https://www.binance.com/en/fee/futureFee); fill-probability: Maglaras et al. 2021 — DL time-to-fill (SSRN), Hollifield et al. — empirical LOB framework (cited 356+); adverse selection crypto: Tiniç et al. 2023 (Bitfinex — thành phần adverse selection của spread đáng kể); practitioner: Multicoin "Adverse Selection Rules Everything Around Me" 2026 (post-only, toxic flow). **Kết luận áp dụng:** fill-rate maker đo **empirically qua replay aggTrades** (trade-through test trên window calibration), không dùng model lý thuyết; adverse selection đo bằng so sánh forward-return sau fill vs sau miss (M4 costs + calibrate_costs.py).
